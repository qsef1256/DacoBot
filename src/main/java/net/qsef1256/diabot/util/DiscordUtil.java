...
public class DiscordUtil {
    ...
    public String getNameAsTag(final long userId) {
        User user = DiaBot.getJda().getUserById(userId);
    ...
    public User getUserFromId(long userId) {
        return DiaBot.getJda().getUserById(userId);
    }

    /**
     * {@link #getUserFromId(long)} 와 같이 작동하지만, 캐시되지 않은 경우 디스코드 API에서 유저를 찾습니다.
     *
     * @param userId User's snowflake id
     * @return matching user
    ...
    public User retrieveUserFromId(long userId) {
        ...
        DiaBot.getJda().retrieveUserById(userId).queue(user::set);
        return user.get();
    }
